...
module ChunkManager
  ...
  MASK_RE = { 
    ACTIVE_CHUNKS => Chunk::Abstract.mask_re(ACTIVE_CHUNKS + [BorkChunk]),
...
class WikiContent < String
  ...
  def initialize(revision, options = {})
    ...
    # hard_breaks are much nicer, put them on by default
    @options[:engine_opts] += [:hard_breaks] if (@web.markup == :textile)
...
=end 
    ...
    gsub!(MASK_RE[ACTIVE_CHUNKS]){ 
      if chunk = @chunks_by_id[$~[1]]
        ...
        $stderr.puts "Not registered chunk: #{$~[0]} "
        $~[0]
      end
      }
    self
